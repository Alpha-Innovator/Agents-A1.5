...
class ScienceMotionFilm {
  constructor(canvas) {
    ...
    this.colors=['#4f1fb2','#1f38b2','#a03177','#349d8e','#b2721f'];
    this.cues=[
      {start:0,end:3.2,id:'question',zh:['小鼠大脑，如何组织不同任务信息？','图案、选择与反馈，怎样在全脑中联系起来？'],en:['How does the mouse brain encode different variables','within a visual decision-making task?']},
      ...
      {start:22.1,end:28.1,id:'closing',zh:['让结果，回答科学问题。','小鼠大脑，如何组织不同任务信息？'],en:['An answer to the scientific question.','Encoding variables within a visual decision-making task.']}
    ];
  ...
  _scaleBar(key,x,y,width=118){
    ...
    this.text(pc1?String(lo):'≤ '+lo,x-width/2-14,y+10,17,'#633f92','right');this.text(pc1?'+'+hi:'≥ +'+hi,x+width/2+14,y+10,17,'#633f92');
    this.text(pc1?'PC1':(LANG==='en'?'Standardized residual':'标准化残差'),x+width/2+86,y+10,17,'#526078');
  }
  _mapStory(t){
    ...
    for(const key of 'FGHIJ'){
      ...
      this.withAlpha(alpha,()=>{
        ...
        c.save();c.shadowColor=key==='F'||key==='G'?'#4e1fb235':'#30a18f25';c.shadowBlur=24; c.drawImage(im,x-w/2,y-h/2,w,h);c.restore();
        const reveal=key==='F'?this.ramp(t,17.4,18.6):key==='G'?this.ramp(t,18.15,19.4):this.ramp(t,19.5+'HIJ'.indexOf(key)*.1,20.9);
        this.withAlpha(Math.sin(reveal*Math.PI)*.6,()=>{const yy=y-h/2+h*reveal;this.line(x-w*.6,yy,x+w*.6,yy,key==='F'||key==='G'?'#591fb2':'#24ad86',1.4)});
        const structural=key==='F'||key==='G';
        const settling=P?1-this.window(t,22.0,22.2,23.2,23.5):1;
        this.withAlpha(settling*this.ramp(t,structural?18.8:20.15,structural?19.3:20.65),()=>this.text(labels[key],x,y-h/2-24,P?25:24,'#693e93','center',450,w+100));
      });
    }
    this.withAlpha(this.ramp(t,20.55,20.9),()=>{
      ...
      this.text(D.resultVersion+(EN?' original colors · IBL / Allen anatomy':' 原图配色 · IBL / Allen 解剖图谱'),P?540:1475,P?1574:969,P?19:17,'#526078','center');
    });
    this.withAlpha(anatomy*(1-fan),()=>this.text('IBL / ALLEN ATLAS',P?540:1335,P?1500:981,P?22:19,'#526078','center'));
  }
  _reportSurface(t){
    const a=this.ramp(t,22.1,23.0),P=this.portrait;
    this.withAlpha(a,()=>{this.box(P?55:62,354,P?970:1796,P?1246:623,26,'#f2f5fae8','#5b2ca55c');
      const c=this.ctx,g=c.createLinearGradient(P?80:95,370,P?1000:1080,920);g.addColorStop(0,'#5d31a013');g.addColorStop(1,'#5d31a000');this.box(P?70:77,369,P?940:1000,P?476:590,18,g,null);
      if(!P)this.line(1090,389,1090,943,'#673c9528');
    });
  }
  _reportCopy(t){
    const a=this.ramp(t,22.5,23.4),P=this.portrait,EN=LANG==='en',x=P?95:105,w=P?895:938;
    this.withAlpha(a,()=>{
      this.text(EN?'RESEARCH BRIEF':'研究结论摘要',x,P?410:408,21,'#653f92','left',450,w);
      const lines=EN?['Task information shares','a regional coding structure','associated with movement.']:['不同任务信息，','呈现与运动相关的','共享编码结构。'];
      lines.forEach((s,i)=>this.text(s,x,(P?489:484)+i*(P?62:59),P?54:53,i===2?'#591fb2':'#611fb2','left',500,w));
      ...
      stats.forEach(([value,label],i)=>{const xx=x+i*w/3,yy=P?701:734;this.line(xx,yy-54,xx+w/3-25,yy-54,'#612aa745');this.text(value,xx,yy,P?43:46,i===1?'#359c7c':'#5921b0','left',450,w/3-24);this.text(label,xx,yy+40,P?21:22,'#663f92','left',400,w/3-25)});
      this.text(EN?'NEXT QUESTION':'下一步研究',x,P?801:857,18,'#3f9270');
      this.text(EN?'Which regional differences should experiments test next?':'哪些脑区的差异，值得进一步实验检验？',x,P?841:898,P?28:31,'#172b45','left',450,w);
      this.text(EN?'Summary of saved results · AGENTS-A1.5 × SEEKBRAIN':'基于本次运行结果整理 · AGENTS-A1.5 × SEEKBRAIN',x,P?876:943,P?18:18,'#526078','left',400,w);
    });
  }
  text(s,x,y,size=28,color='#61349d',align='left',weight=450,maxWidth){
  ...
  box(x,y,w,h,r=14,fill='#f2f5fa',stroke='#5d24ad30'){
  ...
  line(x1,y1,x2,y2,color='#5221b0',width=1){const c=this.ctx;c.strokeStyle=color;c.lineWidth=width;c.beginPath();c.moveTo(x1,y1);c.lineTo(x2,y2);c.stroke()}
  ...
  _background(t){
    const c=this.ctx;c.fillStyle='#f5f8fc';c.fillRect(0,0,this.W,this.H);
    const cx=this.portrait?540:1290,cy=this.portrait?1050:655,g=c.createRadialGradient(cx,cy,0,cx,cy,this.portrait?650:730);g.addColorStop(0,'#6f3e9127');g.addColorStop(.5,'#c6d4e512');g.addColorStop(1,'#f5f8fc00');c.fillStyle=g;c.fillRect(0,0,this.W,this.H);
    for(let i=0;i<90;i++){c.fillStyle=`rgba(82,31,178,${.025+this.hash(i)*.055})`;c.fillRect(this.hash(i+3)*this.W,(this.hash(i+9)*this.H+t*1.2)%this.H,1.1,1.1)}
    c.save();c.translate(cx,cy);c.rotate(-.27+t*.006);for(let j=0;j<3;j++){c.strokeStyle=`rgba(93,55,154,${.095-j*.019})`;c.lineWidth=1;c.beginPath();c.ellipse(0,0,(this.portrait?465:530)+j*40,245+j*32,0,0,Math.PI*2);c.stroke()}c.restore();
  }
  _header(t){
    const P=this.portrait,pad=P?65:85;this.text(D.model.toUpperCase()+' · '+D.resultVersion,pad,62,P?26:22,'#6124ad','left',550);this.text('RESEARCH IN MOTION',this.W-pad,62,P?17:17,'#526078','right');
    ...
    names.forEach((s,i)=>{const x=pad+i*width;this.line(x,111,x+width-16,111,i<=stage?'#5a28aa':'#c6d4e555',i===stage?3:1);this.text(s,x,147,P?20:18,i===stage?'#5a1fb2':'#526078')});
    ...
    this.cues.forEach(cue=>{
      ...
      const copy=cue[LANG==='en'?'en':'zh'];this.withAlpha(a,()=>{
        const c=this.ctx;c.save();c.beginPath();c.rect(pad,178,this.W-2*pad,88);c.clip();this.text(copy[0],pad,242+(1-enter)*100-leave*100,P?46:58,'#6928a9','left',500,this.W-2*pad);c.restore();
        c.save();c.beginPath();c.rect(pad,272,this.W-2*pad,45);c.clip();this.text(copy[1],pad,303+(1-enter)*62-leave*62,P?25:27,'#663f92','left',400,this.W-2*pad);c.restore();
      });
    });
  ...
  _field(t){
    ...
    this.withAlpha(this.window(t,10.7,12,15.9,17.3),()=>{this.text('PC1',P?955:1790,P?1060:660,P?30:28,'#5d1fb2','right');this.line(P?110:900,P?1170:765,P?966:1790,P?1170:765,'#61359c32',1)});
  ...
  _annotations(t){
    ...
    this._note(t,-1,0,2.7,3.8,()=>{
      this.text(EN?'A scientific question':'一个科学问题',x,y,EN?45:47,'#6f359c','left',450,w);
      const lines=EN?['Seeing. Choosing. Receiving feedback.','How are these signals related?']:['看见、选择、获得反馈。','这些信息在全脑中，怎样关联？'];lines.forEach((s,i)=>this.text(s,x,y+72+i*46,P?27:30,'#693f92','left',400,w));
      this.text('AGENTS-A1.5 × SEEKBRAIN',x,y+225,20,'#6a3f92');
    });
    this._note(t,2.7,3.8,6.0,7.0,()=>{
      this.text('59',x,y+72,P?115:145,'#601fb2','left',440);this.text(EN?'provided input files':'个提供的输入文件',x+(P?190:238),y+63,28,'#673f92');
      const fs=[['CSV','region_info.csv','279 × 23'],['NPY','channels.mlapdv.npy','29 × (384 × 3)'],['NPY','channels.brainLocationIds_ccf_2017.npy','29 × 384']];
      fs.forEach(([kind,name,shape],i)=>{const yy=y+145+i*73;this.line(x,yy+45,x+w,yy+45,'#6b3f922b',1);this.text(kind,x,yy+5,18,'#653f92');this.text(name,x+70,yy+5,P?23:21,'#6a3f92','left',420,w-170);this.text(shape,x+w,yy+29,17,'#526078','right')});
    });
    this._note(t,6.0,7.0,9.7,10.8,()=>{
      this.text(EN?'The agent connects the steps.':'让研究，连续向前。',x,y,P?40:43,'#6329a8','left',450,w);
      ...
      labels.forEach((s,i)=>{const yy=y+75+i*77,p=this.ramp(t,6.5+i*.6,7.3+i*.6);this.text(p>.9?'✓':'0'+(i+1),x,yy,25,p>.9?'#3b967f':'#526078');this.text(s,x+50,yy,P?26:27,'#6a3f92','left',420,w-50);this.line(x+50,yy+21,x+50+(w-60)*p,yy+21,'#3f926c50',1.2)});
    });
    this._note(t,9.7,10.8,13.7,14.8,()=>{
      this.text(pct(D.pc1)+'%',x,y+86,P?120:150,'#591fb2','left',440,w);this.text(EN?'Variance captured by one principal axis':'一条主轴，概括的指标总方差',x,y+154,P?27:28,'#723f92','left',400,w);
      const bw=P?720:w;this.box(x,y+207,bw,13,5,'#ecf1f8',null);this.box(x,y+207,bw*D.pc1,13,5,'#5b22af',null);
      this.text((EN?'Shuffled mean: ':'随机参照均值：')+pct(D.nullMean)+'%',x,y+263,24,'#526078');this.text(D.permutations.toLocaleString('en-US')+(EN?' permutations':' 次随机置换'),x,y+307,22,'#526078');
    });
    this._note(t,13.7,14.8,16.7,17.8,()=>{
      this.text(pct(D.r2.choice)+'%',x,y+86,P?120:150,'#339e80','left',440,w);this.text(EN?'Choice-coding variation linked to movement':'选择编码的脑区差异，与运动相关',x,y+154,P?26:28,'#526078','left',400,w);
      this.text(EN?'Variance explained by wheel speed + velocity (R²)':'速度与带方向速度的线性解释比例（R²）',x,y+211,23,'#526078','left',400,w);
      this.text((EN?'Stimulus ':'图案位置 ')+pct(D.r2.stim)+'%   /   '+(EN?'Feedback ':'结果反馈 ')+pct(D.r2.fback)+'%',x,y+287,24,'#526078','left',400,w);
    });
    this._note(t,16.7,17.8,19.0,20.0,()=>{
      this.text(EN?'Locate the organization.':'先看整体结构。',x,y,P?46:46,'#611fb2','left',450,w);
      const lines=EN?['The main axis and a rank-based view','share the same anatomical frame.']:['主要编码轴与排名编码轴，','放回同一套小鼠脑区。'];
      lines.forEach((s,i)=>this.text(s,x,y+85+i*47,P?29:30,'#683f92','left',400,w));
      this.text(EN?'201 regions · Different analytical measures':'201 个脑区 · 不同的分析量',x,y+232,24,'#526078','left',400,w);
    });
    this._note(t,19.0,20.0,21.6,22.6,()=>{
      this.text(EN?'Beyond the movement model.':'再看运动之外。',x,y,P?45:45,'#3b9678','left',450,w);
      ...
      lines.forEach((s,i)=>this.text(s,x,y+85+i*47,P?29:30,'#673f92','left',400,w));
      this.text(EN?'Residual = observed − predicted':'残差 = 观测指标 − 运动模型预测',x,y+285,24,'#526078','left',400,w);
    });
    this._reportCopy(t);
  }
  _footer(t){
    const pad=this.portrait?65:85,y=this.H-43;this.line(pad,y-35,this.W-pad,y-35,'#713f9222',1);this.line(pad,y-35,pad+(this.W-2*pad)*this.clamp(t/28),y-35,'#5c1fb2',2);
    this.text(LANG==='en'?'Saved results · Illustrative data motion':'真实运行产物 · 数据形态与流程示意',pad,y,16,'#526078');this.text(LANG==='en'?'Playback time ≠ model runtime':'播放时长 ≠ 运行耗时',this.W-pad,y,16,'#526078','right');
  }
  render(time){
    const begin=performance.now();this.time=Math.max(0,Math.min(this.duration,time));const c=this.ctx,t=this.time;
    c.setTransform(1,0,0,1,0,0);c.fillStyle='#f5f8fc';c.fillRect(0,0,this.canvas.width,this.canvas.height);c.setTransform(this.scale||1,0,0,this.scale||1,this.ox||0,this.oy||0);
