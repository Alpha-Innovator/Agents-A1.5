...
function drawRwp(){const [c,w,h]=context('rwpChart'),left=43,right=w-18,top=15,bottom=h-28;const x=i=>left+(right-left)*i/8,y=v=>bottom-(v-85.95)/(88.55-85.95)*(bottom-top);
 c.lineWidth=1;for(const v of [86,86.5,87,87.5,88,88.5]){c.strokeStyle='#cbd5e180';c.beginPath();c.moveTo(left,y(v));c.lineTo(right,y(v));c.stroke();c.fillStyle='#526078';c.textAlign='right';c.fillText(v.toFixed(1),left-9,y(v)+3)}
 c.textAlign='center';for(let i=0;i<=8;i++){c.fillStyle=i===selected?'#7952b8':'#667085';c.fillText(String(i),x(i),bottom+17)}
 c.strokeStyle='#b5a3ce';c.setLineDash([3,5]);c.beginPath();F.forEach((f,i)=>i?c.lineTo(x(i),y(f.metrics.rwp)):c.moveTo(x(i),y(f.metrics.rwp)));c.stroke();c.setLineDash([]);
 const cp=checkpointPosition();c.strokeStyle='#7952b8';c.lineWidth=2.2;c.beginPath();c.moveTo(x(0),y(F[0].metrics.rwp));const n=Math.floor(cp);for(let i=1;i<=n;i++)c.lineTo(x(i),y(F[i].metrics.rwp));if(n<8){let q=cp-n;c.lineTo(x(cp),y(F[n].metrics.rwp*(1-q)+F[n+1].metrics.rwp*q))}c.stroke();
 F.forEach((f,i)=>{if(i>cp)return;c.beginPath();c.arc(x(i),y(f.metrics.rwp),i===selected?5:3.5,0,Math.PI*2);c.fillStyle=i&&!f.history.accepted?'#ad642c':'#7952b8';c.fill()});
 if(selected>=4){const bx=w-152,by=17;c.fillStyle='#ffffff';c.fillRect(bx,by,145,61);c.strokeStyle='#cbd5e1';c.strokeRect(bx,by,145,61);c.fillStyle='#526078';c.textAlign='left';c.font='9px Arial';c.fillText('4–8: 86.1198 → 86.1092%',bx+7,by+13);c.beginPath();for(let i=4;i<=Math.max(4,n);i++){const px=bx+12+(i-4)*30,py=by+50-(F[i].metrics.rwp-86.109)/.011*26;i===4?c.moveTo(px,py):c.lineTo(px,py)}c.strokeStyle='#7952b8';c.lineWidth=1.4;c.stroke()}
}
function drawSpectrum(){const [c,w,h]=context('spectrum'),left=43,right=w-12,top=17,bottom=h-48,f=F[selected],obs=D.observed,calc=f.profile;const max=Math.max(...obs,...calc),min=Math.min(0,...obs,...calc),x=v=>left+(right-left)*(v-D.x[0])/(D.x[D.x.length-1]-D.x[0]),y=v=>bottom-(v-min)/(max-min)*(bottom-top);c.font='10px Arial';
 for(let k=0;k<4;k++){const value=min+(max-min)*k/3;c.fillStyle='#526078';c.textAlign='right';c.fillText(value.toFixed(0),left-7,y(value)+3);c.strokeStyle='#cbd5e180';c.beginPath();c.moveTo(left,y(value));c.lineTo(right,y(value));c.stroke()}
 ...
 line(obs,'#667085',y);line(calc,'#7952b8',y);const diff=obs.map((v,i)=>v-calc[i]),dm=Math.max(...diff.map(Math.abs))||1;line(diff,'#0b8f83',v=>h-26-v/dm*12);
