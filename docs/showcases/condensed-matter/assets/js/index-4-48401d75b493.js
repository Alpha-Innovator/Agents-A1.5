(() => {
  ...
  function drawPattern() {
    ...
    ctx.lineWidth = 1;
    [baseline, diffBaseline].forEach(value => { ctx.strokeStyle = '#dce4ef'; ctx.beginPath(); ctx.moveTo(left, value); ctx.lineTo(right, value); ctx.stroke(); });
    ...
    line(D.observed, '#667085', y);
    line(profile, '#7952b8', y, 1.4);
    line(profile.map((v, i) => D.observed[i] - v), '#0b8f83', v => diffBaseline - v / residualMaximum * 12);
    ctx.font = '9px Consolas, monospace'; ctx.fillStyle = '#526078'; ctx.textAlign = 'center';
    ...
    if (inspectedPeak >= 0) {
      ...
      ctx.strokeStyle = '#7952b877'; ctx.setLineDash([2, 3]); ctx.beginPath(); ctx.moveTo(px, top); ctx.lineTo(px, diffBaseline + 8); ctx.stroke(); ctx.setLineDash([]);
      ...
      ctx.fillStyle = '#ffffffee'; ctx.fillRect(tx - 3, top - 5, labelWidth + 6, 16);
      ctx.fillStyle = '#352152'; ctx.textAlign = 'left'; ctx.fillText(label, tx, top + 6);
    }
  }
  ...
  function drawTrace() {
    ...
    ctx.strokeStyle = '#c6b8dc'; ctx.lineWidth = 1; ctx.stroke();
    ctx.beginPath(); for (let i = 0; i <= checkpoint; i++) i ? ctx.lineTo(x(i), y(i)) : ctx.moveTo(x(i), y(i));
    ctx.strokeStyle = '#7952b8'; ctx.lineWidth = 1.4; ctx.stroke();
    ctx.fillStyle = '#7952b8'; ctx.beginPath(); ctx.arc(x(checkpoint), y(checkpoint), 2.5, 0, Math.PI * 2); ctx.fill();
  }
  function localize() {
